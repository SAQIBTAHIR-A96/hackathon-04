export default function Home() {
    ...
    const ProductCard = ({ image, name, price, oldPrice }: ProductCardProps) => {
      return (
        <div
        className="group relative p-4 hover:bg-[aqua] hover:text-white rounded-lg shadow hover:shadow-lg transition"
      >
       <div className="border rounded-lg shadow-md p-4 bg-[#f7f7f7] hover:text-white" >
          <div className="w-360 h-270 mx-auto">
            <img
              src={image}
              alt={name}
              className="w-full h-250 object-cover rounded-t-lg mb-4 hover:text-white" 
            />
          </div>
